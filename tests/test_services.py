...
class ServicesTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    def test_company_workspace_and_graph_aggregate_core_system_state(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            analysis_stream = next(item for item in execution_stream["records"] if item["stream_type"] == "analysis_run")
            self.assertEqual(analysis_stream["meta"]["route"]["path"], "/workspace")
            self.assertEqual(analysis_stream["meta"]["route"]["query"]["run_id"], "run-1")
            runtime_module = next(
                item for item in workspace["runtime_capsule"]["modules"] if item["module_key"] == "analysis"
            )
            self.assertEqual(runtime_module["route"]["path"], "/workspace")
            self.assertEqual(runtime_module["route"]["query"]["run_id"], "run-1")
            history = service.workspace_history(user_role="management", report_period="2025Q3")
            ...
            self.assertEqual(analysis_record["meta"]["route"]["path"], "/workspace")
            self.assertEqual(analysis_record["meta"]["route"]["query"]["run_id"], "run-1")
            self.assertEqual(analysis_record["meta"]["route"]["query"]["role"], "management")
            document_record = next(item for item in history["records"] if item["history_type"] == "document_pipeline")
            self.assertEqual(document_record["meta"]["route"]["path"], "/admin")
            self.assertEqual(document_record["meta"]["route"]["query"]["stage"], "title_hierarchy")
            self.assertEqual(document_record["meta"]["route"]["query"]["report_id"], "demo-report")
