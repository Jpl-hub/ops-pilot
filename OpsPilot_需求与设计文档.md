...
### 19.3 Sprint-3 指标口径冻结（silver v2）

为避免真实数据口径混乱，当前新增以下冻结约束：
...
这意味着当前系统已经进入“真实数据评分可运行，但指标覆盖尚未完整”的阶段。后续开发重点应转向：

1. 利息费用与 EBIT/利润总额口径继续从“近似可用”升级到“附注严格口径”，修正 `S3 / S4`
2. 继续提升 `I2` 的判定严谨度，当前 `2025Q3` 主周期已完成 6/6 公司覆盖，后续重点是把“未经审计 / 标准无保留意见 / 非标意见”边界做得更稳
3. 将 `C3 / S3` 的公式回放进一步产品化，当前已在 UI 中展示卡片化“当前值 / 去年同期 / 差值 / 证据入口”结构，并新增“标签拆解”区承接风险/机会标签、关联指标与证据，后续继续强化交互与视觉表达
4. 真实证据切片与 chunk 级定位，提升审计覆盖率与拒答精度
