...
- 当前已稳定接入的评分输入指标为：`G1 / G2 / G3 / P1 / P2 / P3 / P4 / P5 / C1 / C2 / C3 / S1 / S2 / S4 / I1 / I2 / I3 / I4`
- `P4 / P5` 采用“期初 + 期末平均余额”与报告期天数折算，当前支持 `Q1 / H1 / Q3 / FY`
- `S3` 采用“(利润总额 + 利息费用) / 利息费用”近似口径；当前仅在利润总额可稳定抽取的公司上启用
- `I1` 基于“计入当期损益的政府补助”与利润口径折算；`I3` 基于诉讼/处罚显式披露；`I4` 基于减值压力并兼容异常关联交易关键词
- 当前 `C3` 已通过 2024/2025 同期正式财报完成真实接入；`S3` 仍处于“部分覆盖、继续补齐”阶段

这意味着当前系统已经进入“真实数据评分可运行，但指标覆盖尚未完整”的阶段。后续开发重点应转向：

1. 利息费用与 EBIT/利润总额口径完善，补齐 `S3` 并修正 `S4` 的更严格口径
2. 继续提升 `I2` 的覆盖率，把“未经审计 / 标准无保留意见 / 非标意见”判定做得更稳
3. 为 `C3 / S3` 增加更强的页级证据与公式回放，提升审计可解释性
