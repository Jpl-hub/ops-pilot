...
2. 研报抓取：东方财富个股研报详情页
3. bronze 解析：PDF 页级 block 抽取、chunk 切分、解析 manifest 生成
4. silver 指标抽取：真实财报摘要页 + 资产负债表 + 利润表 + 事件页结构化、单位统一、三季报累计口径统一
5. API 真实数据接管：`score / benchmark / risk-scan / brief` 默认优先走真实 silver 数据
