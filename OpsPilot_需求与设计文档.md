...
- **交互层**：Web UI + API
- **分析层**：Single Agent + Skills + Deterministic Router
- **决策层**：评分引擎、规则引擎、研报核验、证据定位、报告生成
- **可信层**：Audit Guard + 证据回放 + 结构化计算链
- **数据层**：Raw / Bronze / Silver / Gold 四层数据工程

Manager / Fact / Evidence / Diagnosis / Auditor 这些角色只表示系统内部职责边界，不表示面向用户的多个版本或多套系统。
...
9. 禁止首发即微调
10. 禁止将 Agent Teams、OceanBase、Full GraphRAG、全本地大模型作为系统强依赖
...
2. 研报抓取：东方财富个股研报详情页
3. 官方补源：巨潮资讯公司快照 JSON，用于补足深市与历史年报覆盖
4. bronze 解析：PDF 页级 block 抽取、chunk 切分、解析 manifest 生成
5. silver 指标抽取：真实财报摘要页 + 资产负债表 + 利润表 + 事件页结构化、单位统一、三季报累计口径统一
6. API 真实数据接管：`score / benchmark / risk-scan / brief` 默认优先走真实 silver 数据
